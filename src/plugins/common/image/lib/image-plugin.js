...
define([
  ...
  Number.prototype.toInteger =
    Number.prototype.toInteger || String.prototype.toInteger;
  ...
  return Plugin.create("image", {
    ...
    bindInteractions: function () {
      ...
      if (
        plugin.settings.onCropped &&
        typeof plugin.settings.onCropped === "function"
      ) {
      ...
      if (
        ...
        typeof plugin.settings.onReset === "function"
      ) {
      ...
      // Override the default method by using the given one
      if (
        plugin.settings.onResized &&
        typeof plugin.settings.onResized === "function"
      ) {
      ...
      // Override the default method by using the given one
      if (
        plugin.settings.onResize &&
        typeof plugin.settings.onResize === "function"
      ) {
    ...
    subscribeEvents: function () {
      ...
      Aloha.bind(
        ...
        function (event, rangeObject, originalEvent) {
          ...
          if (foundMarkup) {
            // TODO : this is always null (below is dead code, moving it to clickImage)
            plugin.ui._insertImageButton.show();
            plugin.ui.setScope();
            if (plugin.settings.ui.meta) {
    ...
    clickImage: function (e) {
      ...
      cropRatioValue = aspectRatioValue =
        plugin.imageObj.width() / plugin.imageObj.height();
    ...
    findImgMarkup: function (range) {
      ...
      try {
        if (Aloha.activeEditable) {
          if (
            (typeof range.startContainer !== "undefined" &&
              typeof range.startContainer.childNodes !== "undefined" &&
              typeof range.startOffset !== "undefined" &&
              typeof range.startContainer.childNodes[range.startOffset] !==
                "undefined" &&
    ...
    _updateFields: function (primaryFieldName, newValue, isDecrement) {
      ...
      if (typeof plugin.jcAPI !== "undefined" && plugin.jcAPI !== null) {
        adjustedAspectRatio = cropRatioValue
          ? cropRatioValue
          : aspectRatioValue;
      } else {
    ...
    setCropAreaByFieldValue: function () {
      ...
      var selection = [
        currentCropArea.x,
        currentCropArea.y,
        currentCropArea.x + width,
        currentCropArea.y + height,
      ];
    ...
    insertImg: function () {
      ...
      if (range.isCollapsed()) {
        ...
        imagestyle =
          "max-width: " + config.maxWidth + "; max-height: " + config.maxHeight;
        imagetag =
          '<img style="' +
          imagestyle +
          '" src="' +
    ...
    initCropButtons: function () {
      ...
      jQuery("body").append(
        '<div id="aloha-CropNResize-btns" display="none">' +
          '<button class="cnr-crop-apply" title="' +
          i18n.t("Accept") +
          '"></button>' +
          '<button class="cnr-crop-cancel" title="' +
          i18n.t("Cancel") +
          '"></button>' +
          "</div>"
      );
    ...
    _setFocalPoint: function (event) {
      ...
      $("body").trigger("aloha-image-focalpoint", [
        { fpx: fpXFactor, fpy: fpYFactor },
      ]);
    },
    ...
    enableFocalPointMode: function () {
      ...
      if (!jQuery(".aloha-image-box-active").find(".img-overlay-wrap").length) {
        jQuery(".aloha-image-box-active").append(
